    ...
    <div className="bg-slate-50 text-slate-900 pb-24 md:pb-0 font-sans">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-8 text-sm font-semibold text-slate-600">
            ...
            <button
              ...
              className="bg-blue-600 text-white px-5 py-2.5 rounded-full hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all active:scale-95"
            >
              Book Now
            </button>
          </nav>
      ...
      <section className="relative px-6 py-12 md:py-24 max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12">
        <div className="flex-1 space-y-6 text-center md:text-left">
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold leading-tight text-slate-900">
            Expert Care, <span className="text-blue-600">Simplified</span> For
            You.
          </h1>
          <p className="text-lg text-slate-600 max-w-lg mx-auto md:mx-0">
            Connect with top-rated specialists and manage your medical
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4 justify-center md:justify-start">
            ...
            <button
              ...
              className="bg-white border border-slate-200 px-8 py-4 rounded-2xl font-bold hover:bg-slate-50"
            >
              View Specialists
            </button>
          </div>
      ...
      <section id="departments" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <p className="text-slate-500 mb-12">
            Find specialized care for every health need
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-6">
            {departmentsData.map((dep) => (
              <div
                key={dep.id}
                className="group cursor-pointer p-6 rounded-3xl border border-slate-100 hover:border-blue-200 hover:bg-blue-50/50 transition-all"
              >
                <div
                  className={`w-16 h-16 ${dep.color} rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4 group-hover:scale-110 transition-transform`}
                >
                  {dep.icon}
                </div>
      ...
      <section id="doctors" className="py-20 bg-slate-50 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-end mb-10">
            ...
            <button
              onClick={() => handleProtectedAction("/patient/book-doctor")}
          ...
          <div className="flex gap-6 overflow-x-auto no-scrollbar pb-8 snap-x">
            {data.topDoctors.map((doc) => (
              <div
                key={doc._id}
                className="min-w-[300px] snap-center bg-white rounded-3xl p-4 border border-slate-200 hover:shadow-xl transition-all"
              >
                <img
                ...
                <div className="px-2">
                  ...
                  <button
                    onClick={() =>
                      handleProtectedAction(`/patient/doctors/${doc._id}`)
                    }
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {data.topReviews.map((rev) => (
              <div
                key={rev._id}
                className="bg-slate-50 p-8 rounded-3xl border border-slate-100 relative group"
              >
                <Quote
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-lg border-t border-slate-200 md:hidden z-[100] px-4 py-3 shadow-2xl">
        <div className="flex justify-between items-center max-w-md mx-auto">
          ...
          <button
            onClick={() => handleProtectedAction("/login")}
            className="flex flex-col items-center gap-1 text-slate-400 hover:text-blue-600"
          >
            <User size={20} />{" "}
